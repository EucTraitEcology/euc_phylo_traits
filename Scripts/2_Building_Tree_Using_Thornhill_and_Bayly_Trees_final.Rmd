...
The following notebook uses Andrew Thronhill's (Thornhill et al. 2019) tree of ~675 Eucalypt taxa and Bayly et al.'s (2013) tree to make a combined supertree of as many taxa in our dataset as are included on them (plus closest relatives of others where useful). Taxa for which we have trait data and are not already on the tree will be positioned on it using other smaller phylogenies and taxonomic information to complete the tree for any subsequent PGLS analysis and measurement of phylogenetic signal.

This script also produces Figure 2 (or the stage of its creation before manual additions) of the manuscript (summary heatmap of traits alongside phylogeny; exported filename 'heatmap_colorblind_abridged.pdf') as well as supplementary Table S3.2 (summary of justification of eventual positioning of each phylogenetically ambiguous taxon; no exported file, manually created in ms word) and supplementary Figures S5 (pdf of final version phylogeny with certain subgeneric groups labelled; exported filename 'labelled_tree.pdf') and S6 (summary heatmap like Figure S2 but with the taxon labels still present; exported filename 'heatmap_colorblind.pdf'). Together with scripts 3a and 3, this script is also necessary for creating the data for supplementary Table S3.1 (comparison of lambda values of 15-17 different possible trees to assess effect of different ambiguous taxon positions on analysis; not exported until the end of script 3a).

Note: the version of the Bayesion tree from Thornhill's 2019 paper that we're using can be found at: https://data.csiro.au/collection/csiro:33546v2. The 'Eucalypt_Bayes.tre' file that we use here contains a tree that has negative branch lengths due to the low statistical support for certain branches. However, since we remove branch lengths entirely before merging the input trees and finalising the form of the phylogeny we will use in later analyses, the negative branch lengths do not affect our analysis and have been verified to create the same final output as other versions of the bayesian tree without negative branch lengths i.e. 'Eucalypt_Bayes_rooted.tre'.
...
all.taxa.final <- read.csv('../Input_data/all_taxa_final.csv')
all.taxa.final <- all.taxa.final[c('taxon')]
```
...
# function specifically for the instance where we need to remove all tips that were extra taxa for keeping for positioning
...
The following section seeks to check the names of taxa on both the Bayly and Thornhill trees in order to make sure that differences in naming conventions don't cause taxa to be treated as seperate entities when they are in fact the same taxon. This was done by using the Australian Plant Names Index (APNI) and Australian Plant Census (APC) name-checking function (found here https://biodiversity.org.au/nsl/services/search/name-check) to determine which names on the Bayly and Thornhill trees were no longer current. Only taxa appearing on both trees and taxa with older synonyms appearing on the other tree that also have a synonym or current name in the trait dataset were looked at. This is because: (1) taxa not in the trait dataset or synonyms of taxa on the trait dataset will be pruned away eventually anyway as they are not necessary for our analysis; and (2) taxa that are only in one of the two phylogenies (assuming none of their synonyms are on the other tree) that will later be merged must only match the way that name is written in our trait dataset as there is no danger of creating duplicate tips for the same taxon upon merging the trees.

Since not every name has been manually checked after looking through its taxonomic history and determining the circumscription of the current name, there is a chance that names that are identical and current on different trees and the trait dataset may refer to a different subset of individuals or real world populations.
...
# for namechecking in APC site, need to export list of names
write.csv(bayly.tree$tip.label, file = "../Intermediate_Tree_Building_Products/bayly_names_for_check.csv")
```

All names current on the Bayly Tree except E. deglupta, which similarly does not show up anywhere on APNI. However, it is on the Thornhill tree and considered an accepted name with Blume as the authority. Either way it will be pruned away later due to not being in the trait dataset.
...
# intermediate files such as name_check_thorntips.csv have ; taxathorntips_for_check.csv and thorntips_for_check2.csv have the list of 20 taxa on the thorntree that were found using APC to have more current names
```

Once the 'thornhill_taxa_for_APC.csv' file is exported to the Intermediate_Tree_Building_Products folder, it can be used as a species list to bulk check the correctness of names according to the APC using APNI's name check function.

The full output of the name-check in APC of the full 675 taxa  can be found in 'name_check_thorntips.csv' (in the Intermediate_Tree_Building_Products folder).

Of the many taxon names listed as 'not found' by the APC namecheck (most of the non-current ones are like this), some are simply no longer current. Not all of the taxon names on the thornhill tree are (as currently written or their newer names) in the trait dataset anyway so many will be removed eventually at later steps.
...
thorntips.check <- as.data.frame(read.csv("../Intermediate_Tree_Building_Products/name_check_thorntips.csv", header = TRUE))
...
write.csv(non.current.thorntips2, file = "../Intermediate_Tree_Building_Products/final_thorntips_for_check.csv", row.names = FALSE)

# checking how many of our trait taxa match thornhill-tree taxa (thorntips.for.nomcheck contains all thornhill taxa before they were name-checked), 110 taxa are in this category
sum(all.taxa.final$taxon %in% thorntips.for.nomcheck) 
## the following indicates there are 54 (was 56 before) taxa in the trait dataset that are not on the thornhill tree at all
trait.taxa.nonthorn <- data.frame('taxon' = all.taxa.final$taxon[!all.taxa.final$taxon %in% thorn.tree$tip.label])
## the following indicates that, of the 54 (was 58) taxa not on the thorntree, only one of those is on the bayly tree instead (E. aromaphloia)
...
Thorn tips non-current and without synonym listed:
  ...
  - E. caesia subsp. caesia -> E. caesia (in neither other tree, so making sure the form of the name matches anything is unnecessary)
  - E. argyphea -> E. falcata (already elsewhere on thorntree)
  - E. communalis (APC Excluded) -> no longer distinct and now considered intergrade between E. adesmophloia and E. obesa (neither of which are in either of the other trees)
  ...
  - E. oblonga -> globoidea or sparsifolia (E. globoidea in traits dataset and this taxon likely represents that one rather than the latter because there's already E. sparsifolia elsewhere in the thorntree, but E. globoidea already exists as a seperate tip on the tree anyway so it doesn't require action)
...
So from this above investigation, only E. piperita subsp. piperita needs to be changed to E. piperita on grounds of taxonomy and inclusion/relevance in the traits dataset. Especially considering we will be pruning down the thornhill tree before merging it with the bayly tree, so taxa not included in that would not be relevant. Where there was already another tip with the current name, that existing tip can be used to represent the position of that trait-taxon.

```{r}
# taxonomic change of taxon on thornhill tree relevant to trait taxon list
...
The following on the above list of 28 'not found' taxa are relevant to our trait dataset and will now be changed:
...
Need to figure out how many of the 11 non-matching (with the thorntree) taxa in the Bayly tree may not match simply because of different levels of resolution in the two trees. How many of those 11 taxa have member of the same species (but maybe different subspecies) on the thorntree?
...
All of those 9 taxa are indeed those with subspecies epithets and since, there are only 9, using only the species name shouldn't create too much more work later to add the subspecies epithets back in after merging the trees.
...
## following line gives the species names for taxa in the trait dataset that do not have species epithets and don't match the thornhill tree
...
## the following gets rid of the repeated species names from the trait dataset contaning multiple subspecies from the same species (reduces number down to 44 now once following line is run)
...
# created a column of all the trait taxa with subspecies epithets, 62 taxa in total
...
# creating a column of the 102 species in the trait dataset that DO NOT have subspecies epithets
trait.taxa.species <- data.frame('taxon' = all.taxa.final$taxon[!grepl("subsp", all.taxa.final$taxon)])
...
# the following gives the species epithets of the remaining 3 taxa that are not in the trait dataset 
thorntip.spp.for.nmt.bayly.spp[!thorntip.spp.for.nmt.bayly.spp %in% trait.taxa.spp.for.sspp]
```

There are 6 taxa within these 9 that have subspecies on the traits dataset (of the remaining 3, 2 aren't even in the traits dataset and the last is E. aromaphloia, which will match the species in the trait dataset anyway) that will need to be changed back later to match the trait dataset.
...
Allosyncarpia ternata -> not on thorntree but also not in trait dataset as it was simply a different outgroup and so will be pruned later anyway
Stockwellia quadrifida -> not on thorntree but also not in trait dataset anyway like the one above
...
Many of the taxa in the trait dataset are resolved down to a different taxonomic level than the trees (i.e. subspecies when the tree only goes down to species level, or only down to species level when the trees go down to subspecies). 

To check if the Thornhill tree ever has more than one representative of each species i.e. multiple subspecies sampled:

```{r}
# none that aren't repeated species epithets in different genera (i.e. in Angophora or Corymbia) so we know that only species in different genera have the same species epithets and there is only one representative/tip of each of the sampled species on the tree.
sum(duplicated(thorn.tip.spp))
thorn.tip.spp$species[duplicated(thorn.tip.spp)]
```


Since it appears that the Thornhill tree only includes a single member of each species even when there are multiple subspecies known, there is some assumption of species monophyly being made, especially in the cases where Thornhill tips are not resolved down to subspecies at all. As a result, it is not particularly egregious for us to make the assumption with this available information, that any member of a species can be used to represent the position of all members of that species i.e. species are monophyletic.

Now to test which of the non-matching subspecies have species/other_subspecies that are simply resolved differently on the thorn tree or bayly tree. This may not be the case for all species with no matches, but it is the case with the vast majority.

First to redo the name-checking with the traits dataset to see if the 18 taxa are still there:

```{r}
# now to create a new list of trait taxa that don't match the thornhill tree names; 56 taxa now, 54 taxa we had before + the 6 taxa that have now been changed but minus the 4 thornhill taxa (E. piperita, E. doratoxylon, E. ovata + E. aromaphloia) that were changed to match the trait dataset
non.matching.trait.taxa2 <- data.frame('taxon' = all.taxa.final$taxon[!all.taxa.final$taxon %in% thorn.tips2$taxon]) 
...
## only three new names bc the other three taxa (that make up the six that now don't match) already had subspecies that didn't match and so were already on the list as species and the above function only returns completely new names and not new duplicates of existing names
...
## The following lists the 28 unique species epithets that are repeated to generate the 40 total
nmt.spp.for.sspp2$species[duplicated(nmt.spp.for.sspp2$species)]
unique(nmt.spp.for.sspp2$species)

## Each of the 28 unique species appears the following number of times (i.e. how many subspecies of that species do we have in the non-matching trait taxa list)
sum(nmt.spp.for.sspp2[nmt.spp.for.sspp2$species %in% thorn.tip.spp2$species,] == "globulus") # 4
sum(nmt.spp.for.sspp2[nmt.spp.for.sspp2$species %in% thorn.tip.spp2$species,] == "radiata") # 2
sum(nmt.spp.for.sspp2[nmt.spp.for.sspp2$species %in% thorn.tip.spp2$species,] == "costata") # 2 (second from Angophora not Eucalyptus though, so not true subspecies in this case)
...
These 40/41 non-matching trait subspecies that have species epithets on the thorn tree (Syncarpia glomulifera subsp. glomulifera does not have any match at all on the thorn tree so will have to be dealt with and added to the final tree manually) can be positioned as follows:

  - Firstly, once the two trees are merged, we will rename/add those 6 names back to what they are in the trait dataset in order to get the tree tip labels to match the trait dataset:
  ...
  - For the rest:
      ...
      - E. viminalis (there are 2) can be attached as sister to the existing E. viminalis subsp. viminalis
...
Now for the non-subspecies trait taxa that don't match (i.e. trait taxa that Thornhill resolved down to subspecies but the trait dataset does not):
...
# now extracting their species epithets similarly to what was done for the other non-matching trait taxa, note there are no duplicates in this list of 15 (was this 28 ever?)
nmt.spp.for.spp2 <- data.frame("species" = str_split_fixed(non.matching.trait.SPECIES$taxon, " ", 4)[,2])
# determining which of these species has a subspecies (another member of the species) on the thornhill tree, there are 3 and then the remaining 12 (used to be 17?) do not have species-level matches at all
nmt.spp.for.spp2[nmt.spp.for.spp2$species %in% thorn.tip.spp2$species,]
nmt.spp.for.spp2[!nmt.spp.for.spp2$species %in% thorn.tip.spp2$species,]
```


For these 3/20 trait species that have subspecies on the thorn tree, said subspecies can be renamed to get rid of the subspecies epithet in order to find the position of these species on the tree (i.e. we can use the position of the subspecies to decide where the species goes on he tree since we're assuming species monophyly):

  - E. willisii subsp. willisii -> E. willisii
  - E. uncinata subsp. uncinata -> E. uncinata
  - E. socialis subsp. socialis -> E. socialis

Since we're assuming species monophyly and both input trees have only a single tip for each species sampled, we don't have to worry about some species having more multiple representatives in different places in the tree and then be uncertain where a new subspecies taxon should be placed bc the species itself is non-monophyletic; and the assumption that any subspecies is a representative of its species can hold and we can replace any subspecies with another or the species alone. 
...
For the 13 ((these 12 + Syncarpia) from the subspecies list that didn't even match a species) that have no match of even species level:
...
- Syncarpia glomulifera subsp. glomulifera: Not sure just yet. Need more information.





The list of `all.taxa.clean` imported at the beginning of this script has an extra 5 taxa compared to `all.taxa.final`. This would include the following 4 taxa for which we have some trait data but have subsequently decided to exclude based on either: there not being enough phylogenetic information, or not enough actual trait data to justify investigating further into methods of attempting to place them:

- E. bunyip: nothing on APC but Nicolle considers it to be intergrade between E. camphora subsp. humeana and E. strzeleckii (both are taxa that either partially match through different subspecies or have only series-mate inference respectfully; hence can't place this species until E. strzeleckii is placed). VicFlora has no synonyms listed for it, it was defined by Rule, but E. strzeleckii and E. camphora are noted as taxa it can be distinguished from). It is noted as a synonym but not considered distinct enough to be included in EUCLID.

- E. carolaniae: no APC info but Nicolle condiers it an intergrade between E. cypellocarpa and E. goniocalyx subsp. goniocalyx (both on tree and trait dataset). Vicflora has it having a synonym of E. aff. cypellocarpa (Mornington Peninsula) so maybe its closer to E. cypellocarpa than it is to E. goniocalyx and can be positioned accordingly). It is nowhere to be found on the EUCLID database.
...
- 2 taxa (bunyip and carolaniae) are considered intergrades between other taxa often not sister to each other. Not even considered to be seperate entities by many official sources and are hence difficult to position.

- 2 taxa (dendromorpha and robusta) have some relatives known but not much phylogenetic information.

The 5th and final taxon for which we have trait data but have excluded from this analysis was the potential outgroup taxon, Tristaniopsis laurina. Given the importance of deep divergences in this analysis, there was insufficient trait data (only a single immature indivdual sampled at the time of writing) on this species to use it as such.





Hence, of the 12 above + Syncarpia glomulifera:
...
- 2 taxa are the outgroups Lophostemon and Syncarpia, which will require some digging in non-eucalypt literature to determine where they should be (none of the three taxon lists have any outgroups in common so noting where they fall for the trees to be merged after they are merged is not likely to be a reliable method to see how non-eucalypt taxa end up)
...
(Already part of subspecies changing above so no further action required for this species) E. cinerea subsp. triplex (current) is on the Thornhill tree so will need to be changed back to the obsolete name E. cinerea subsp. victoriensis to match the trait dataset again after trees have been merged (so that trait data will attach correctly)

The new name E. incrassata is on the Thornhill tree and so a sister taxon for E. costata subsp. murrayana will need to be added once the trees are done, since E. incrassata is also a separate taxon on the traits dataset, despite these names being considered synonyms in other official sources.

(already accounted for by above process) For E. decipiens ssp. chalara, the ssp. decipiens is on the Thornhill tree, but there's no E. adesmophloia between which this is an intergrade, so this taxon will be added as a sister taxon to the E. decipiens tip.
...
# making sure non of these names are redundant and already in the trait dataset, should also return 'character(0)'
extra.taxa.for.keeping$taxon[extra.taxa.for.keeping$taxon %in% all.taxa.final$taxon]
```
...
It is worth noting that outgroups such as Syncarpia glomulifera subsp. glomulifera, Lophostemon confertus will be positioned will be later. 



## Final Thornhill

After all checking of names and mismatches caused by differences in taxonomic resolution, spelling variation, or the lack of direct representatives of trait taxa on the Thornhill tree, we actually prune the Thornhill tree down to the smallest set of taxa necessary for our analysis before merging it with the Bayly tree.


```{r}
# first to attach the trait dataset and bayly tree taxa together and extract duplicates, it should contain 203 (was 208) names including the duplicates, as we want to make sure none of the taxa on the bayly tree get excluded from the pruned thornhill tree even if they're not in the trait dataset
taxa.for.keeping <- as.data.frame(rbind(all.taxa.final, bayly.tips))
## the follow column contains 188 (was 193) names
...
# the following should be 219 (was 224) names long
...
# without duplicates this should be 217 (was 222) names long, after we get rid of the 8 duplicated taxa
...
# there are 217 (was 222) names currently in this list of trait taxa + the species we will use to position most of those that don't have a direct match
length(all.taxa.for.keeping.clean$taxon)

# 159 taxa of these 217 (217-57?) (was 222 (222-63)) match a tip on the thornhill tree the rest are trait taxa that we've included close relatives or other subspecies for to anchor them
sum(all.taxa.for.keeping.clean$taxon %in% thorn.tips2$taxon)
```
...
## first lets deal with the 6 aforementioned taxa from lines 428-434 and 521-528
...
## now for the rest of the trait taxa with subspecies epithets but species-level matches on the thornhill tree (lines 531-554)
...
```{r fig.height=25, fig.width=15}
#now for the near-matched species-only traits taxa with members of the same species on the thorntree (lines 572-576)
...
Now to handle the remaining mismatches left that we have agreed solutions for:
...
all.taxa.final$taxon[!all.taxa.final$taxon %in% merged.tree$tip.label] #once E. costata subsp. murrayana is dealt with (below) then there are only the 11 taxa (9 taxa plus the 2 outgroup taxa) (was 16 = 11 + 3 OG taxa + 2 about to place?)
```
...
E. wimmerensis (only synonym E. viridis subsp. wimmerensis listed as vicflora synonym, this taxon was defined by Rule) (Rule was a taxonomist particularly fond of splitting groups into separate species where many others would not have done so)

E. silvestris (this taxon was defined by Rule and considered by VicFlora to be somewhat intermediate between E. microcarpa and E. wimmerensis, also considered to be a depauperate but distinguishable form of E. microcarpa)
...
E. imitans a close relative of E. baxteri, which is on the traits dataset and already in the tree. Hence, we can use that species as its closest relative and place it sister to it!
...
# now to look at where our samples of E. behriana were collected to see if that sheds any light on the matter
unique(combined_traits[combined_traits$taxon == 'Eucalyptus behriana', c('locality')])
```


E. strzeleckii closest to E. yarraensis, E. ovata and E. brookeriana (latter has its autonym ssp. on the thornhill tree and the others are allready on the tree)
...
Now also to remember to add E. costata murrayana as a sister taxon to E. incrassata, since Nicolle treats them as a single taxon indicating they are likely closer to each other than they are to other taxa, though APC considers them to both be current names and hence seperate.
...
Now that we know what to keep as potential placeholders for these other 10 non-outgroup taxa (E. punctata - 1st 3 in following list, E. wimmerensis - next 1 (2nd one already on tree), E. silvestris - relative of E. wimmerensis and E. microcarpa latter on the tree allready, E. behriana - next 2, E. strzeleckii - E. brookeriana sp., E. dendromorpha - last 2 before outgroup. Note we keep one of the outgroups on the tree to help position our outgroups on an existing node.
...
# just checking the list above has no spelling mistakes and they're all on the taxon list for the current growing tree we're working with (i.e. should return 'character(0))
extra.taxa.for.keeping2[!extra.taxa.for.keeping2$taxon %in% merged.tree$tip.label,]

merged.tree$tip.label <- gsub("_", " ", merged.tree$tip.label)
taxa.for.keeping2 <- data.frame("taxon" = rbind(all.taxa.final, extra.taxa.for.keeping2))
# checking there are no duplicates, there appears to be none
sum(duplicated(taxa.for.keeping2$taxon))

# now to prune away all the close relative we kept on the thornhill tree that we know we are no longer going to be using as we've narrowed some of them down. Note: taxa that had a match at species level on the Thornhill tree have at this point been changed to match the version of the name that is in the trait dataset, hence why all the taxa we need are in the taxa.for.keeping2 object with the trait dataset list plus the much smaller list of close relatives.
...
# to export the newick file of the tree to read it back in later
write.tree(final.tree1, file = "../Intermediate_Tree_Building_Products/final_tree1.txt")
```
...
# prunes all 14 potential trees down to only the taxa we have trait data for using a custom function and not any that were kept purely to help position some of the more uncertain taxa
...
saveRDS(pot.trees.pruned.nwk, file = "../Intermediate_Tree_Building_Products/Potential_trees_nwk_list.RData")
```

This list of 14 trees are not all combinations of every possible position of each of the uncertain taxa, but should encompass the greatest variation in position for taxa that have particularly extreme values for particular traits. Hence, this list can be used now to determine the phylogenetic signal for all 10 traits in the final dataset and what the value of correlations were for SLA-LA and SD-MH (these were two relationships whose statistical significance we know from preliminary analysis to have been most sensitive to whether or not phylogeny was accounted for).
This will have to be run through scripts 3a to actually look at the results for each of these trees before deciding and running the remaining sections of this script.


### Finalising Tree

So after determining: that the different positions of even the most extreme-valued dubious taxa (those whose position had been narrowed down at least to 2-4 positions) didn't greatly affect the measures of phylogenetic signal (see results of script 3a); deciding its ok to only use Patrick Fahey's tree to place taxa not already on the thornhill tree (and otherwise ignore its mild conflict about other taxa that are on it already), and deciding that it's ok to exclude E.dendromorpha and E. robusta bc they only have 2-4 individuals measured and no reproductive trait data, we need to place wimmerensis, silvestris and behriana where Patrick's tree says they go.
...
## to read it back in, just in case the environment got cleared during the scrip switching
final.tree1 <- read.tree("../Intermediate_Tree_Building_Products/final_tree1.txt")
final.tree1$tip.label <- gsub("_", " ", final.tree1$tip.label)
plot_dim1(final.tree1)
...
final.tree <- keep.tip(final.tree2, final.tree2$tip.label[final.tree2$tip.label %in% all.taxa.final$taxon])

## now the tree should have 164 tips corresponding to the 164/169 trait taxa that we have managed to determine a reasonable position for
...
```{r fig.height = 25, fig.width = 15}
# we need to know the numbers of each of the nodes we wish to label
...
Now to plot visualise this data as a heatmap, plotting 3 different heatmaps for each group of traits: max height and stem sapwood density in one group; SLA, leaf area and leaf mass in the next; and fruit wall width, fruit mass, seed mass in the last. This is so it easier to interpret patterns and read the map because it creates spaces between blocks of 2-3 related traits. Also data was log-transformed (base 10) and scaled so that comparisons between traits with very different values and variances could be made.
...
# to read tree back in so we don't have to rerun this whole notebook to run this chunk
...
# the baseline tree with enough space to the right to fit all of the heatmaps to be attached, this includes labels for infrageneric classifications/internal nodes, taxon names, and colouring tip ends by fire response
...
# see if the parameters are correct, everything visible on the page etc. Do be careful that the final pdf will have different dimensions though so may not look the same.
# plot(map.height)
...
# just to keep the original colorblind colors for fire response dots on tips of tree in case we need an alternative
...
# the baseline tree with enough space to the right to fit all of the heatmaps to be attached, this includes labels for infrageneric classifications/internal nodes and colouring tip ends by fire response. But, no taxon names this time.
