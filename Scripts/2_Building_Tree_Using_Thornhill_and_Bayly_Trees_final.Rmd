...
The following notebook uses Andrew Thronhill's (Thornhill et al. 2019) tree of ~675 Eucalypt taxa and Bayly et al.'s (2013) tree to make a combined supertree of as many taxa in our dataset as are included on them (plus closest relatives of others if useful). Taxa for which we have trait data and are not already on the tree will be positioned on it using other smaller phylogenies and taxonomic information to complete the tree for any subsequent PGLS analysis and measurement of phylogenetic signal.

This script also produces Figure 2 of the manuscript (summary heatmap of traits alongside phylogeny) as well as supplementary Table S3.2 (summary of justification of eventual positioning of each phylogenetically ambiguous taxon) and supplementary Figures S5 (pdf of final version phylogeny with certain subgeneric groups labelled) and S6 (summary heatmap like Figure S2 but with the taxon labels still present). Together with scripts 3a and 3, this script is also necessary for creating the data for supplementary Table S3.1.

Note: the version of the Bayesion tree from Thornhill's 2019 paper that we're using can be found at: https://data.csiro.au/collection/csiro:33546v2. The 'Eucalypt_Bayes.tre' file that we use here contains a tree that has negative branch lengths due to the low statistical support for certain branches. However, since we remove branch lengths entirely before merging the input trees and finalising the form of the phylogeny we will use in later analyses, the negative branch length do not affect our analysis and are equivalent to other versions of the bayesian tree without negative branch lengths for the purposes of this analysis i.e. 'Eucalypt_Bayes_rooted.tre'.
