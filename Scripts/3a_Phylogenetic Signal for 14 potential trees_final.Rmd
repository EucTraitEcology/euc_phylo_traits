...
The script acts like one large single function to be rerun for each potential phylogeny. Each time the sections down to the end of 'Dot-Plot of Lambda values' is run choose a different number to put in line 69 to select a particular tree from that list of 15 and simply change the name of the output figure (line 194) and the final object lambda.full (line 187) to generate and store the result for each tree run to match the tree selected for that run. Rerun for each tree in the list of potential trees being tested.
...
Just a quick check to test that the differences are mainly due to updates in the dataset and not the additional taxa. I will prune down our tree back to the 160 taxa we had before adding the extra 4.

```{r}
tree.test1 <- read.tree(file = '../Input_data/final_tree.txt')
tree.test1$node.label <- NULL
tree.test1$tip.label <- gsub("_", " ", tree$tip.label)
tree.test1 <- drop.tip(tree.test1, c('Corymbia eximia', 'Eucalyptus viridis', 'Eucalyptus goniocalyx subsp. viridissima', 'Eucalyptus leucoxylon subsp. pruinosa', 'Eucalyptus socialis'))

eucs.test.tree <- tree.test1
eucs.test.tree <- compute.brlen(eucs.test.tree, method = "Grafen", power = 1)
eucs.scaled.obj <- comparative.data(phy = eucs.test.tree, data = eucs.scaled.data, names.col = taxon, vcv = TRUE, na.omit = FALSE, warn.dropped = TRUE)


sla.la.prel.test1 <- pgls(sla_mm2_per_mg ~ leaf_area_cm2, data = eucs.scaled.obj, lambda = 'ML')
summary(sla.la.prel.test1)
mh.sd.prel.test1 <- pgls(max_height_m ~ stem_density_g_per_ml, data = eucs.scaled.obj, lambda = 'ML')
summary(mh.sd.prel.test1)
```
